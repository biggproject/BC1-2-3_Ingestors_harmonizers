...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description='Launching longitudinal benchmarking module')
    ...
    existing_hashes = set()
    hash_building_map = {}
    query_set = [sensor_query, tariff_query, co2_query]

    for graph in buildings_rdf:
        for query in query_set:
            b_hashes = get_hashes_from_rdf(graph, query)
            existing_hashes.update([list(k.keys())[0] for k in b_hashes])
            for b_hash in b_hashes:
                current_hash = list(b_hash.keys())[0]
                if current_hash not in hash_building_map:
                    hash_building_map[current_hash] = set(b_hash[current_hash])
                else:
                    hash_building_map[current_hash].update(b_hash[current_hash])

    tar = tarfile.open("buildings_hash.tgz", 'w|gz')
    for hash_id, building_set in hash_building_map.items():
        with open(f"{hash_id}.json", "w") as f:
            json.dump({hash_id: list(building_set)}, f)
        tar.add(f"{hash_id}.json")
    tar.close()
    print(time.time()-s)
