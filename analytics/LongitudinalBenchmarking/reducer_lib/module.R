...
save_data <-function(buildingId, buildingsRdf, timeseriesObject){
    example_file = paste0(buildingId,".RData")
    save(list(buildingsRdf=buildingsRdf,timeseriesObject=timeseriesObject), file=example_file)
    ...
    resp1 = httr::PUT(paste0(uri_1,mpath,example_file,uri_2))
    httr::PUT(resp1$url, body=setNames(list(file=httr::upload_file(example_file)),example_file))
}
...
launch_module <- function(buildingsRdf, timeseries){
    ...
    for (buildingSubject in allBuildingSubjects){
      ...
      tryCatch({
        ...
        if(buildingSubject == "https://icaen.cat#BUILDING-01047"){
            write(sprintf("reporter:status: SAVING %s", buildingSubject),stderr())
