...
get_building_rdf<-function(building_subject){
  for (f in list.files(pattern=".*.ttl")){ 
    write(f, file=stderr())
    buildingsRdf <- rdf_parse(get_subgraph_from_rdf(f, building_subject) , format="turtle")
...
reduce = function(key, values){
  ...
  for (file in ts_shared_list){
    ...
    table = read.table(text=resp, sep=";", header=FALSE, col.names=c("start", "end", "value", "isReal"))
