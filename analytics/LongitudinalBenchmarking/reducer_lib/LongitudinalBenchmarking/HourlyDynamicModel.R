HourlyDynamicModel <- function(df, identifier, settings, tz, plots=T, updateHadoopStatus=F){
  ...
  write(updateHadoopStatus, stderr())
  ...
  if(needsToBeTrained){
    ...
    if(updateHadoopStatus==T){
        write(sprintf("reporter:status: REUSING MODEL %s", identifier),stderr())
    }
    ...
    df_result <-
      if(nrow(df)==0){
        ...
        if( (
          ...
          write("* Detecting the time period with affectance by SARS-CoV2 lockdowns",stderr())
          if(updateHadoopStatus==T){
            write(sprintf("reporter:status: DETECTING SARS 1 %s", identifier),stderr())
          }
          minIniDate = max(min(as.Date(df$time[is.finite(df$Qe)], tz=tz)) +
                             months(settings$DataCleaning$CheckForDisruption$minInitialMonths) ,
                           as.Date(settings$DataCleaning$CheckForDisruption$minIniDate))
        ...
        write("* Detecting the holidays",stderr())
        if(updateHadoopStatus==T){
            write(sprintf("reporter:status: DETECTING OUTLIERS 1 %s", identifier),stderr())
        }
        holidaysDates <- detect_holidays_in_tertiary_buildings(
          data = df, 
          consumptionColumn = "Qe", 
        ...
        write("* Detecting the outliers",stderr())
        if(updateHadoopStatus==T){
            write(sprintf("reporter:status: DETECTING OUTLIERS 2 %s", identifier),stderr())
        }
        if(all(c("value","window") %in% colnames(df)))
          df <- df %>% select(-value, -window)
        df <- 
  ...
  if(nrow(df_result)==0){
    ...
    write("Success!",stderr())
    if(updateHadoopStatus==T){
        write(sprintf("reporter:status: SUCCESS 1 %s", identifier),stderr())
    }
    write("",stderr())
