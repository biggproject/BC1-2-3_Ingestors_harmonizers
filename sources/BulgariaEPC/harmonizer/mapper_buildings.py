...
def clean_dataframe_building_info(df_orig, source):
    ...
    df['location_subject'] = df['building_id'].str.split("-").apply(lambda x: "-".join(x[1:])).apply(location_info_subject)
    ...
    df['net_area_subject'] = df['building_id'].str.split("-").apply(lambda x: "-".join(x[1:]))\
        .apply(partial(net_area_subject, a_source=source))
    df['heated_area_subject'] = df['building_id'].str.split("-").apply(lambda x: "-".join(x[1:]))\
        .apply(partial(heated_area_subject, a_source=source))
    df['cooled_area_subject'] = df['building_id'].str.split("-").apply(lambda x: "-".join(x[1:]))\
        .apply(partial(cooled_area_subject, a_source=source))
    return df
