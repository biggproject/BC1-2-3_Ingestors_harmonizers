...
#         utils.utils.log_string(f"Obtaining from {date_ini_req} to {date_end_req}", mongo=False)
#
...
class DatadisMRJob(MRJob, ABC):
    ...
    def mapper(self, _, line):
        ...
        pkgs = freeze.freeze()
        for pkg in pkgs: print("pkg", file=sys.stderr)
    ...
    #         id_key = 0
    #         utils.utils.log_string(f"Obtained: {len(supplies)} supplies", mongo=False)
    ...
    # def reducer(self, key, values):
        ...
        #     try:
        #         date_end = datetime.strptime(supply['validDateTo'][:-2], '%Y/%m').date()
        #         if date_end <= now:
        #             date_end = date_end
        #             finished = True
        #         else:
